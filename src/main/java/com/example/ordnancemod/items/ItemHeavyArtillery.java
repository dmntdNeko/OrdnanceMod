...
public class ItemHeavyArtillery extends Item {
    private static final String __OBFID = "CL_00001774";

    public ItemHeavyArtillery()
    {
        this.maxStackSize = 1;
    }
        ...
        else
        {
            ...
            else
            {
                if (movingobjectposition.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK)
                {
                    ...
                    EntityHeavyArtillery entityHeavyArtillery = new EntityHeavyArtillery(world, (double)((float)i + 0.5F), (double)((float)j + 1.0F), (double)((float)k + 0.5F));
                    entityHeavyArtillery.rotationYaw = (float)(((MathHelper.floor_double((double)(player.rotationYaw * 4.0F / 360.0F) + 0.5D) & 3) - 1) * 90);

                    if (!world.getCollidingBoundingBoxes(entityHeavyArtillery, entityHeavyArtillery.boundingBox.expand(-0.1D, -0.1D, -0.1D)).isEmpty())
                    {
                        return stack;
                    }
                    ...
                    if (!world.isRemote)
                    {
                        world.spawnEntityInWorld(entityHeavyArtillery);
                    }
